...
Public Class frmCCtaCteContratista
    Public Ls_Permisos As New List(Of Integer)
    Dim Ls_Reporte As List(Of ETCuentaCorriente) = Nothing

    Private Sub frmCCtaCteContratista_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Call Inicio()
    End Sub
    ...
    Public Sub Buscar()
        ...
        Txt1.Text = Trim(Proveedor.Codigo & "")
        Txt2.Text = Trim(Proveedor.RazonSocial & "")
        Txt4.Text = Trim(Proveedor.Ruc & "")
        Proveedor = Nothing
    End Sub
    ...
    Public Sub Procesar()
        ...
        If String.IsNullOrEmpty(Txt1.Text.Trim) Then
            MsgBox("Seleccione al Contratista o Proveedor", MsgBoxStyle.Exclamation, msgComacsa)
            Exit Sub
        End If
        ...
        Entidad.MyLista = Negocio.CtaCteContratista.Consultar_CtaCorrienteContratista(Txt1.Text.Trim, Txt3.Value)
        If Entidad.MyLista.Validacion Then
            Ls_Reporte = Entidad.MyLista.Ls_CtaCteContratista
        End If
    ...
    Public Sub Excel()
        ...
        Path = My.Application.Info.DirectoryPath & "\Reporte de Cta Cte Contratistas\"

        Archivo = Path & "Reporte de Cta Cte Contratistas - " & Txt1.Text & " DEL " & Txt3.Value & ".xls"
        UltraGridExcelExporter1.Export(Grid1, Libro, 2, 0)
        Libro.Worksheets(0).Name = "Cta Cte Contratistas"
        ...
        Libro.Worksheets(0).MergedCellsRegions.Add(0, 0, 0, 10).CellFormat.FillPatternForegroundColor = Color.Aqua
        ...
        Libro.Worksheets(0).Rows(1).Cells(8).Value = "RUC"
        Libro.Worksheets(0).Rows(1).Cells(9).Value = Txt4.Text.Trim
        Libro.Worksheets(0).MergedCellsRegions.Add(1, 9, 1, 10)
        ...
        Libro.Worksheets(0).Columns(6).CellFormat.FormatString = "#,##0.00"
        Libro.Worksheets(0).Columns(7).CellFormat.FormatString = "#,##0.00"
        Libro.Worksheets(0).Columns(8).CellFormat.FormatString = "#,##0.00"
    ...
    Private Sub Grid1_InitializeLayout(ByVal sender As System.Object, ByVal e As Infragistics.Win.UltraWinGrid.InitializeLayoutEventArgs) Handles Grid1.InitializeLayout
        ...
        For Each uColumn As UltraGridColumn In e.Layout.Bands(0).Columns
            If Not (uColumn.Key = "ID" OrElse uColumn.Key = "Fecha" _
                    OrElse uColumn.Key = "Debe" OrElse uColumn.Key = "Haber" _
                    OrElse uColumn.Key = "Saldo" OrElse uColumn.Key = "VoucherConta" _
                    OrElse uColumn.Key = "Voucher" OrElse uColumn.Key = "Documento" _
                    OrElse uColumn.Key = "Cantidad" OrElse uColumn.Key = "Numero" _
                    OrElse uColumn.Key = "Lote" OrElse uColumn.Key = "Detalle") Then
