...
Imports System.IO
Imports System.Text
Public Class FrmRptVacacionesLiquidaciones
...
#End Region
    ...
    Private Sub FrmRptVacacionesLiquidaciones_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Dim mes As Int32
        mes = Month(Now.Date)
        Dim fecha1 As Date
    ...
    Private Sub CargarDatos()
        Try
            ...
            dtVacaciones = Negocio.NContratista.RptVacaciones_CTR(Entidad.Contratista)
            dtLiquidaciones = Negocio.NContratista.RptLiquidaciones_CTR(Entidad.Contratista)
            If dtVacaciones.Rows.Count <= 0 And dtLiquidaciones.Rows.Count <= 0 Then
                MsgBox("No existen datos para mostrar", MsgBoxStyle.Information, msgComacsa)
                Exit Sub
            End If
            imprimir(dtVacaciones, dtLiquidaciones)
    ...
    Sub imprimir(ByVal dtVacaciones As DataTable, ByVal dtLiquidaciones As DataTable)
        Dim m_Excel As New Microsoft.Office.Interop.Excel.Application
        Try
            Dim path As String
            path = Application.StartupPath
            File.Delete(path & "\RPT_VACIONES_LIQUIDACIONES_" & txtcodcontratista.Text.Trim & ".xls")
            File.Copy(RutaReporteERP & "PLANTILLAVACIONESLIQUIDACIONES_.xls", path & "\RPT_VACIONES_LIQUIDACIONES_" & txtcodcontratista.Text.Trim & ".xls")
            m_Excel.Workbooks.Open(path & "\RPT_VACIONES_LIQUIDACIONES_" & txtcodcontratista.Text.Trim & ".xls")
            ...
            If dtLiquidaciones.Rows.Count > 0 Then
                ...
                nfila = 0
                With objHojaExcel
                    For i As Int16 = 0 To dtLiquidaciones.Rows.Count - 1
                        Dim nfilaini As Integer
                        Dim nfilault As Integer = 0
                        nfilaini = 5
                        .Activate()
                        .Range("B" & nfilaini + i).Value = dtLiquidaciones.Rows(i)("ITEM")
                        .Range("C" & nfilaini + i).Value = dtLiquidaciones.Rows(i)("CONTRATISTA")
                        .Range("D" & nfilaini + i).Value = dtLiquidaciones.Rows(i)("CANTERA")
                        .Range("E" & nfilaini + i).Value = dtLiquidaciones.Rows(i)("TRABAJADOR")
                        .Range("F" & nfilaini + i).Value = dtLiquidaciones.Rows(i)("NUMSOLICITUD")
                        .Range("G" & nfilaini + i).Value = dtLiquidaciones.Rows(i)("AYO")
                        .Range("H" & nfilaini + i).Value = dtLiquidaciones.Rows(i)("MES")
                        .Range("I" & nfilaini + i).Value = dtLiquidaciones.Rows(i)("FECHA_DEPOSITO")
                        .Range("J" & nfilaini + i).Value = dtLiquidaciones.Rows(i)("IMPORTE")
            ...
            If dtVacaciones.Rows.Count > 0 Then
                Dim objHojaExcel1 As Microsoft.Office.Interop.Excel.Worksheet = m_Excel.Worksheets(2)
                Dim nfila As Integer
                nfila = 0
                With objHojaExcel1
                    For i As Int16 = 0 To dtVacaciones.Rows.Count - 1
                        Dim nfilaini As Integer
                        Dim nfilault As Integer = 0
                        nfilaini = 5
                        '.Activate()
                        .Range("B" & nfilaini + i).Value = dtVacaciones.Rows(i)("ITEM")
                        .Range("C" & nfilaini + i).Value = dtVacaciones.Rows(i)("CONTRATISTA")
                        .Range("D" & nfilaini + i).Value = dtVacaciones.Rows(i)("CANTERA")
                        .Range("E" & nfilaini + i).Value = dtVacaciones.Rows(i)("TRABAJADOR")
                        .Range("F" & nfilaini + i).Value = dtVacaciones.Rows(i)("NUMSOLICITUD")
                        .Range("G" & nfilaini + i).Value = dtVacaciones.Rows(i)("AYO")
                        .Range("H" & nfilaini + i).Value = dtVacaciones.Rows(i)("MES")
                        .Range("I" & nfilaini + i).Value = dtVacaciones.Rows(i)("PERIODO")
                        .Range("J" & nfilaini + i).Value = dtVacaciones.Rows(i)("FECHA_DEPOSITO")
                        .Range("K" & nfilaini + i).Value = dtVacaciones.Rows(i)("IMPORTE")
                        .Range(.Cells(nfilaini + i, 2), .Cells(nfilaini + i, 13)).Borders.LineStyle = Microsoft.Office.Interop.Excel.XlLineStyle.xlContinuous
                        nfila = nfilaini + i
                    Next
                    .Range("K" & nfila + 2).Value = "=SUMA(K5:K" & nfila & ")"
                    .Range("K" & nfila + 2).Font.Bold = True
                    .Range("K" & nfila + 2).NumberFormat = "###,##0.00"
                    .Range("K" & nfila + 2).Borders(Microsoft.Office.Interop.Excel.XlBordersIndex.xlEdgeTop).LineStyle = Microsoft.Office.Interop.Excel.XlLineStyle.xlContinuous
                    .Range("K" & nfila + 2).Borders(Microsoft.Office.Interop.Excel.XlBordersIndex.xlEdgeBottom).LineStyle = Microsoft.Office.Interop.Excel.XlLineStyle.xlDouble
                End With
            End If
